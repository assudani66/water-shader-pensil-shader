...
export const WatercolorShader = {
    ...
    uniforms: {
        ...
        "tPaper":		{ value: null }, // paper texture
        "texel":		{ value: new THREE.Vector2(1.0 / 512, 1.0 / 512) },
        "scale":		{ value: 0.03 }, // wobble scale
        "threshold":	{ value: 0.7 }, // edge threshold
        "darkening":    { value: 1.75 }, // edge darkening
        "pigment":      { value: 1.2 }, // pigment dispersion
    },
    ...
    fragmentShader: `
        ...
        void main() {
            ...
            vec4 color = pd * texture2D(tDiffuse, uv);
            
            if (edge > threshold) {
                 color = pd * edgeDarkening(tDiffuse, uv);
            }
